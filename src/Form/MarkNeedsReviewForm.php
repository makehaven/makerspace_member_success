...
/**
 * Confirms no-contact "Needs Review" status updates from stage queues.
 */
class MarkNeedsReviewForm extends ConfirmFormBase {
  ...
  public function getFormId(): string {
    return 'makerspace_member_success_mark_needs_review_form';
  }
  ...
  public function getQuestion(): string {
    ...
    return (string) $this->t('Mark @member as "Needs Review" for @stage stage without logging an interaction?', [
      '@member' => $label,
      '@stage' => $stage,
    ]);
  }
  ...
  public function getDescription(): string {
    return (string) $this->t('This saves the member for later review, suppresses current queue work for this stage, and leaves a clear revisit marker.');
  }
  ...
  public function getConfirmText(): string {
    return (string) $this->t('Mark Needs Review');
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state): void {
    ...
    $updated = $this->followupStatusManager->applyNoContactStatusByMemberStage(
      ...
      'manual_needs_review_no_contact'
    );

    if ($updated) {
      $this->messenger()->addStatus($this->t('Follow-up status updated to Needs Review for @member.', [
